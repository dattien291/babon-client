import {
  ...
  selectTotalComments,
} from '@/kytesoft-client/store/product/slice';
import {
  createCommentThunk,
  getCommentsThunk,
  getReplyCommentsThunk,
} from '@/kytesoft-client/store/product/thunks';
...
export const useProductComments = ({ id, messages = {} }) => {
  ...
  const commentForm = useFormik({
    initialValues: { content: '', images: [], rating: 0 },
    validationSchema: commentSchema(messages),
    onSubmit: async ({ content, images, rating }, { setSubmitting }) => {
      await dispatch(createCommentThunk({ id, content, isReply, commentId, images, rating }));
      setSubmitting(false);
    },
  });
